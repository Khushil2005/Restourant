...
export const BillingPage: React.FC<BillingPageProps> = ({ defaultTab = 'invoices' }) => {
  ...
  const handleGenerateBillForOrder = async (
    ...
    if (!isServed) {
      alert(
        `Cannot generate bill: Order #${orderNumber || orderId} is currently '${status || 'IN_KITCHEN'}'.\n\nAll items must be SERVED to the table before generating a bill.\n(ઓર્ડર કિચનમાંથી સંપૂર્ણ સર્વ થયા પછી જ બિલ જનરેટ કરી શકાય છે).`
      );
      return;
    }
            ...
            >
              ...
              {settings.showFooterNote && (
                <div className="text-center text-muted mt-1" style={{ fontSize: '0.68rem' }}>
                  <p className="mb-0">{settings.customFooterText || 'Thank you for dining with us! Please Visit Again 🙏'}</p>
                </div>
              )}
            </div>
                    ...
                    <div className="card border shadow-sm h-100">
                      <div className="card-body p-2.5 d-flex flex-column justify-content-between">
                        ...
                        <div className="d-flex gap-1">
                          <button
                            className={`btn ${isServed ? 'btn-primary' : 'btn-outline-secondary'} btn-sm py-1 w-100 d-flex align-items-center justify-content-center gap-1 shadow-sm fw-bold`}
                            style={{ fontSize: '0.75rem' }}
                            onClick={() => handleGenerateBillForOrder(order.id, tbl.tableNumber, order?.orderNumber, order?.status, order?.totalAmount)}
                            disabled={isGenerating || !isServed}
                            title={isServed ? 'Generate Tax Invoice' : `Order is ${order?.status} - Must be SERVED in kitchen before billing`}
                          >
                            {isGenerating ? (
                              <>
                              ...
                              <>
                                <Receipt size={13} /> {isServed ? 'Generate Bill' : `Awaiting Kitchen (Not Served)`}
                              </>
                            )}
                          </button>
                        </div>

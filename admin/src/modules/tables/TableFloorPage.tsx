...
export const TableFloorPage: React.FC = () => {
  ...
  const handleExecuteMerge = async (e: React.FormEvent) => {
    ...
    if (selectedMergeTableIds.length < 2) {
      alert('Please select at least 2 tables to merge (ઓછામાં ઓછા 2 ટેબલ સિલેક્ટ કરો).');
      return;
    }
  ...
  // Filter out secondary / child tables so merged tables show as strictly ONE unified card
  const displayedTables = tables.filter(t => !t.isMergedChild);

  const filteredTables = selectedZone === 'ALL'
    ? displayedTables
    : displayedTables.filter(t => t.floorZone === selectedZone);
    ...
    <div className="d-flex flex-column gap-3 gap-sm-4 pb-4" style={{ overflowX: 'hidden' }}>
      ...
      <div className="d-flex flex-wrap justify-content-between align-items-center gap-2">
        <div>
          ...
          <p className="text-muted small mb-0">Visual seating plan, live table statuses, order turnover, table merging and transfers</p>
        </div>
        <div className="d-flex align-items-center gap-2 flex-wrap">
          {can('tables.merge') && (
            <button
              ...
              title="Merge Multiple Tables for Extra Seating (ટેબલ ભેગા કરો)"
            >
        ...
        <div className="d-flex flex-wrap justify-content-between align-items-center gap-3">
          <div className="nav nav-pills gap-1 scrollable-pills-container flex-grow-1" style={{ minWidth: 0 }}>
            ...
            {activeZoneList.map(z => {
              const count = displayedTables.filter(t => t.floorZone === z.code).length;
              return (
                <button
                  key={z.code}
              ...
              >
                <div className={`card-header ${isMergedPrimary ? 'bg-warning-subtle' : `bg-${color}-subtle`} border-0 d-flex justify-content-between align-items-center p-2 px-sm-3 gap-1`}>
                  <div className="d-flex align-items-center gap-1 text-truncate">
                    ...
                    <span className="fw-bold text-dark font-monospace fs-6 fs-sm-5 text-truncate" title={isMergedPrimary && table.mergedTableNumbers?.length ? table.mergedTableNumbers.join(' + ') : table.tableNumber}>
                      {isMergedPrimary 
                ...
                <div className="card-body p-2 p-sm-3 d-flex flex-column justify-content-between" style={{ minHeight: '135px' }}>
                  <div>
                    ...
                    <div className="d-flex align-items-center gap-1 mb-2 overflow-hidden flex-wrap" style={{ maxWidth: '100%' }}>
                      ...
                      >
                        ...
                        <span className="fw-bold">
                          {isMergedPrimary ? `${table.mergedCapacity || table.capacity} Seats (Merged)` : `${table.capacity} Seats`}
                        </span>
                      </span>
                  ...
                  <div className="d-flex flex-wrap gap-1 mt-auto pt-2 border-top">
                    ...
                    {table.status === 'AVAILABLE' && can('orders.create') && (
                      <>
                        <button
                          ...
                          onClick={() => {
                            const targetNumber = isMergedPrimary && table.mergedTableNumbers?.length 
                              ? table.mergedTableNumbers.join(' + ') 
                              : table.tableNumber;
                            navigate(`/pos?tableId=${table.id}&tableNumber=${encodeURIComponent(targetNumber)}`);
                          }}
                        >
                        ...
                        {can('tables.merge') && !isMergedPrimary && (
                          <button
                            ...
                            title="Merge this table with other tables for extra seating (ટેબલ મર્જ કરો)"
                          >
                            <Link2 size={13} />
                          </button>
                        )}
                      ...
                      <>
                        <button
                          ...
                          onClick={() => {
                            ...
                            navigate(`/pos?tableId=${table.id}&tableNumber=${encodeURIComponent(targetNumber)}${orderParam}`);
                          }}
                        >
                    ...
                    {/* UNMERGE ACTION BUTTON FOR MERGED PRIMARY */}
                    {isMergedPrimary && can('tables.split') && (
                      <button
                        ...
                        title="Unmerge tables back to individual available tables (ટેબલ અલગ કરો)"
                      >
                        <Unlink size={13} />
                      </button>
                    )}
      ...
      <Modal
        ...
        title="Merge Tables for Large Parties (ટેબલ ભેગા કરો)"
      >
        <form onSubmit={handleExecuteMerge} className="d-flex flex-column gap-3">
          ...
          <div>
            ...
            <div className="row g-2" style={{ maxHeight: '260px', overflowY: 'auto' }}>
              {tables
                .filter(t => (mergeZoneFilter === 'ALL' || t.floorZone === mergeZoneFilter) && !t.isMergedChild && !t.isMerged)
                .map(table => {

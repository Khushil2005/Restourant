...
export class BillingService {
  ...
  static async generateBillFromOrder(orderId: string, options: any = {}, userId?: string, username?: string) {
    ...
    // Strict Validation: Order must be fully SERVED before generating a bill
    if (order.status !== 'SERVED' && order.status !== 'BILLED') {
      throw {
        statusCode: 400,
        message: `Cannot generate bill: Order #${order.orderNumber || order.id} is currently '${order.status}'. All items must be SERVED to the table before generating a bill (ઓર્ડર સંપૂર્ણ સર્વ થયા પછી જ બિલ જનરેટ કરી શકાય છે).`
      };
    }

    // Double check that no active unserved KOT tickets remain
    const unservedKOTs = await KOTTicket.countDocuments({
      orderId: order.id,
    ...
    if (unservedKOTs > 0) {
      throw {
        statusCode: 400,
        message: `Cannot generate bill: Order #${order.orderNumber || order.id} has ${unservedKOTs} unserved kitchen item(s). All dishes must be served to the table before billing.`
      };
    }
